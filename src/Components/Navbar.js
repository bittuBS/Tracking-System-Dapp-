...
function Navbar({currentUser,connectWallet}){
...
width={120}  height={50} alt="float ui logo" />
</a>
<div className="md:hidden">
    <button  className="menu-btn text-grey-500 hover:text-gray-800"
   ...
   <div className={`flex-1 items-center mt-8 md:mt-0 md:flex ${
    ...
    <ul className="justify-center items-center space-y-6 md:flex md:space-x-6">
        {navigation.map((item,idx)=>{
            return (<li key={idx} className="text-gray-700 hover:text-grey-900">
...
<div  className="flex-1 gap-x-6 items-center justify-end mt-6 space-y-6 md:flex md:space-y-0 md:mt-0">
    {currentUser? (<h2 className="flex items-center justify-center gap-x-1 py-2 px-4
     text-white  font-medium bg-gray-800 hover:bg-gray-700 
      active:bg-gray-900 rounded-full md:inline-flex">
         {currentUser.slice(0,25)}...
    </h2>):(
        <button  onClick={()=>connectWallet()} className="flex items-center justify-center gap-x-1 py-2 px-4
     text-white  font-medium bg-gray-800 hover:bg-gray-700 
      active:bg-gray-900 rounded-full md:inline-flex"> 
      Connect Wallet
       </button>)}
